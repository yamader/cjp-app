...
StrDict? _dictCommon, _dictPNoun, _dictKana, _dictKanji, _dictEmoji;

class CJP {
  static Future<bool> loadDict() async {
    if (!_isDictReady) {
      await Future.wait([
        rootBundle.loadString("assets/cjp.js/dict/common.json").then((d) {
          _dictCommon = jsonDecode(d);
        }),
        rootBundle.loadString("assets/cjp.js/dict/propernoun.json").then((d) {
          _dictPNoun = jsonDecode(d);
        }),
        rootBundle.loadString("assets/cjp.js/dict/kana.json").then((d) {
          _dictKana = jsonDecode(d);
        }),
        rootBundle.loadString("assets/cjp.js/dict/kanji.json").then((d) {
          _dictKanji = jsonDecode(d);
        }),
        rootBundle.loadString("assets/cjp.js/dict/emoji.json").then((d) {
          _dictEmoji = jsonDecode(d);
        }),
      ]);
      _isDictReady = true;
    }
    ...
    for (var i = 0; i < matched.length; i++) {
      ...
      if (data.processedIdx.any((d) =>
        ...
        if (correctStr.length != incorrectStr.length) {
          ...
          data.processedIdx = data.processedIdx
              .map((d) => cursor < d.item1
